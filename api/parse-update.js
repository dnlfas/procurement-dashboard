...
Rules:
- PRIORITY 1 — BTS order confirmation (so_file): If the document (image, PDF, or text) contains a title or heading "אישור הזמנה מספר SO..." or "אישור הזמנה מס' SO..." — this is a BTS-issued Sales Order confirmation sent TO a customer. Set sourceType to "so_file". DO NOT classify as "customer_po" — BTS is the SENDER here, not the customer. Extract: so from the SO number in the title (e.g. "SO26000131"), mpn from the "מק"ט לקוח" column (this is the customer's part number — use it as mpn). If "מק"ט לקוח" is empty for a row, use "מק"ט" instead. Put the "מק"ט" value (BTS catalog code) in supplierRef when it differs from mpn. deliveryDate from "תאריך אספקה" (ISO YYYY-MM-DD), qty from "כמות" or "יתרה לאספקה". Skip service/test line items where both "מק"ט לקוח" and "מק"ט" are empty or contain Hebrew service descriptions like "בדיקה", "מעבדה", "שירות". Leave custPO, customer, tracking, notes empty.
